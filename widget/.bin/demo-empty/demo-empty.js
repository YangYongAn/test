(function() {
	...
	var Empty = /*#__PURE__*/ (function(_Component2) {
		_inheritsLoose(Empty, _Component2);
		function Empty() {
			var _this2;
			for (
				var _len2 = arguments.length, args = new Array(_len2), _key2 = 0;
			...
			_defineProperty(_assertThisInitialized(_this2), "render", function(props) {
				...
				var _props = props,
					...
					color = _props.color;
				var imageWidth =
					(_ref = imageStyle && imageStyle.width) != null ? _ref : fontSize;
				var imageColor =
					(_ref2 = imageStyle && imageStyle.color) != null ? _ref2 : color;
				...
				return apivm.h(
					...
					apivm.h(
						"div",
						{className: classPrefix2$1 + "-image-container"},
						imageNode()
					),
					props.description &&
						apivm.h(
							"span",
							{style: "color:#ccc;font-size:14px;margin-top:8px"},
							props.description
						)
				);
			});
	...
	var DemoEmpty = /*@__PURE__*/ (function(Component) {
		function DemoEmpty(props) {
			Component.call(this, props);
		}

		if (Component) DemoEmpty.__proto__ = Component;
		DemoEmpty.prototype = Object.create(Component && Component.prototype);
		DemoEmpty.prototype.constructor = DemoEmpty;
		DemoEmpty.prototype.back = function() {
			api.closeWin();
		};
		DemoEmpty.prototype.render = function() {
			return apivm.h(
				"safe-area",
				null,
				apivm.h(nav_bar_default, {onBack: this.back}, "Empty"),
				apivm.h(
					"scroll-view",
					null,
					apivm.h(
						DemoBlock,
						{title: "基础用法", padding: "0"},
						apivm.h(empty_default, null)
					),

					apivm.h(
						DemoBlock,
						{title: "描述文字", padding: "0"},
						apivm.h(empty_default, {description: "暂无数据"})
					),

					apivm.h(
						DemoBlock,
						{title: "自定义样式", padding: "0"},
						apivm.h(empty_default, {
					...
					apivm.h(
						DemoBlock,
						{title: "自定义图片", padding: "0"},
						apivm.h(
							empty_default,
							{style: "padding: '64px 0'", description: "暂无数据"},
							apivm.h(icon_default, {fontSize: 60, color: "#9ac200"})
						)
					)
				)
			);
	...
	DemoEmpty.csslink = [
		...
		"../../components/avm-ui/empty/index.css"
	];
	apivm.define("demo-empty", DemoEmpty);
	apivm.render(apivm.h("demo-empty", null), "body");
})();
