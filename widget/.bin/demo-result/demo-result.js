(function() {
	...
	var iconRecord = {
		success: apivm.h(
			Icon$2,
			_extends({code: 59931}, iconDefaultProps["success"])
		),
		error: apivm.h(Icon$2, _extends({code: 59938}, iconDefaultProps["error"])),
		info: apivm.h(Icon$2, _extends({code: 59984}, iconDefaultProps["info"])),
		waiting: apivm.h(
			Icon$2,
			_extends({code: 59937}, iconDefaultProps["waiting"])
		),
		warning: apivm.h(Icon$2, _extends({code: 59955}, iconDefaultProps["warning"]))
	};

	var Result = /*#__PURE__*/ (function(_Component2) {
		_inheritsLoose(Result, _Component2);
		function Result() {
			var _this2;
			for (
				var _len2 = arguments.length, args = new Array(_len2), _key2 = 0;
			...
			_defineProperty(_assertThisInitialized(_this2), "render", function(props) {
				...
				var resultIcon = iconRecord[status];
				if (children) {
					if (typeof children[0] === "string" && children[0].includes(".")) {
						var fontSize = iconDefaultProps[status].fontSize;
						resultIcon = apivm.h("img", {
				...
				return apivm.h(
					"div",
					{className: classNames(classPrefix2$1, classPrefix2$1 + "-" + status)},
					apivm.h("div", {className: classPrefix2$1 + "-icon"}, resultIcon),
					apivm.h("text", {className: classPrefix2$1 + "-title"}, title),
					description
						? apivm.h(
								"text",
								{className: classPrefix2$1 + "-description"},
								description
						  )
						: null
				);
	...
	var DemoResult = /*@__PURE__*/ (function(Component) {
		function DemoResult(props) {
			Component.call(this, props);
		}

		if (Component) DemoResult.__proto__ = Component;
		DemoResult.prototype = Object.create(Component && Component.prototype);
		DemoResult.prototype.constructor = DemoResult;
		DemoResult.prototype.back = function() {
			api.closeWin();
		};
		DemoResult.prototype.render = function() {
			return apivm.h(
				"safe-area",
				null,
				apivm.h(nav_bar_default, {onBack: this.back}, "Result"),
				apivm.h(
					"scroll-view",
					null,
					apivm.h(
					...
					apivm.h(
						...
						apivm.h(
							result_default,
							{
								...
								title: "Well done",
								description:
									"内容详情可折行，建议不超过两行建议不超过两行建议不超过两行"
							},
							apivm.h(icon_default, {fontSize: 52, color: "#1677ff"})
						)
					)
				)
			);
	...
	DemoResult.csslink = [
		...
		"../../components/avm-ui/result/index.css"
	];
	apivm.define("demo-result", DemoResult);
	apivm.render(apivm.h("demo-result", null), "body");
})();
