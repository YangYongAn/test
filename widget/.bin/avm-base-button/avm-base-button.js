(function() {
	...
	var AvmBaseButton = /*@__PURE__*/ (function(Component) {
		function AvmBaseButton(props) {
			Component.call(this, props);
		}

		if (Component) AvmBaseButton.__proto__ = Component;
		AvmBaseButton.prototype = Object.create(Component && Component.prototype);
		AvmBaseButton.prototype.constructor = AvmBaseButton;
		AvmBaseButton.prototype.back = function() {
			api.closeWin();
		};
		AvmBaseButton.prototype.btnAction = function(e) {
			api.toast({
				msg: "触发了点击事件"
			});
		};
		AvmBaseButton.prototype.render = function() {
			return apivm.h(
				"safe-area",
				{class: "page"},
				apivm.h("avm-nav-bar", {onBack: this.back}, "Button"),
				apivm.h(
					"scroll-view",
					...
					apivm.h(
						DemoBlock,
						{title: "基础使用"},
						apivm.h("button", {onclick: this.btnAction}, "默认按钮")
					),
					apivm.h(
						DemoBlock,
						{title: "禁用"},
						apivm.h("button", {onclick: this.btnAction, disabled: true}, "禁用按钮")
					)
				)
			);
		};

		return AvmBaseButton;
	})(Component);
	AvmBaseButton.css = {
		".page": {width: "100%", height: "100%", background: "#f2f2f2"}
	};
	apivm.define("avm-base-button", AvmBaseButton);
	apivm.render(apivm.h("avm-base-button", null), "body");
})();
