(function() {
	...
	var AvmBaseInput = /*@__PURE__*/ (function(Component) {
		function AvmBaseInput(props) {
			Component.call(this, props);
			this.data = {};
		}

		if (Component) AvmBaseInput.__proto__ = Component;
		AvmBaseInput.prototype = Object.create(Component && Component.prototype);
		AvmBaseInput.prototype.constructor = AvmBaseInput;
		AvmBaseInput.prototype.back = function() {
			api.closeWin();
		};
		AvmBaseInput.prototype.onconfirm = function(e) {
			api.alert({
				msg: "搜索" + e.detail.value
			});
		};
		AvmBaseInput.prototype.render = function() {
			return apivm.h(
				"safe-area",
				{class: "page"},
				apivm.h("avm-nav-bar", {onBack: this.back}, "Input"),
				apivm.h(
					DemoBlock,
					{title: "基础用法"},
					apivm.h("input", {placeholder: "请输入"})
				),
				apivm.h(
					DemoBlock,
					{title: "输入字数限制"},
					apivm.h("input", {
						placeholder: "输入限制10个字符",
						maxlength: "10",
						autofocus: true
					})
				),
				apivm.h(
					DemoBlock,
					{title: "密码输入框"},
					apivm.h("input", {type: "password", placeholder: "请输入"})
				),
				apivm.h(
					DemoBlock,
					{title: "数字键盘"},
					apivm.h("input", {placeholder: "请输入", "keyboard-type": "number"})
				),
				apivm.h(
					DemoBlock,
					{title: "键盘完成按钮文字为搜索"},
	...
	AvmBaseInput.css = {
		...
		input: {margin: "8px", width: "auto", padding: "0 8px"}
	};
	apivm.define("avm-base-input", AvmBaseInput);
	apivm.render(apivm.h("avm-base-input", null), "body");
})();
