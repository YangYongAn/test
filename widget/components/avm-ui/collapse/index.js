...
var CollapsePanel = class extends Component {
  render = () => {
    return null;
  };
};
var CollapsePanelContent = class extends Component {
  render = (props) => {
    const { visible } = props;
    return <div className={`${classPrefix4}-panel-content`} style={{ height: visible ? "auto" : 0 }}><ListItem>{visible && formatLabel(props.children, `${classPrefix4}-panel-content-text`, {})}</ListItem></div>;
  };
};
var Collapse = class extends Component {
  ...
  render = (props) => {
    ...
    if (this.data.isInit) {
      this.data.activeKey = props.activeKey || props.defaultActiveKey ? !props.accordion ? props.activeKey || props.defaultActiveKey : [props.activeKey || props.defaultActiveKey] : props.accordion ? null : [];
    }
    const activeKeyList = this.data.activeKey === null ? [] : Array.isArray(this.data.activeKey) ? this.data.activeKey : [this.data.activeKey];
    return <div className={classPrefix4}><List>{panels.map((panel) => {
      const key = panel.key;
      ...
      return <div key={panel.key}>
        <ListItem {...restProps} className={`${classPrefix4}-panel-header`} disabled={panel.disabled} handleClick={panel.disabled ? void 0 : handleClick} arrow={panel.arrow || (active ? <Icon_default code={60049} color="#bbb" /> : <Icon_default code={59949} color="#bbb" />)}>{formatLabel(panel.title)}</ListItem>
        <CollapsePanelContent visible={active}>{panel.content || panel.children}</CollapsePanelContent>
      </div>;
    })}</List></div>;
  };
  css = () => {
