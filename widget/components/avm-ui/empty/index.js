  ...
  render = (props) => {
    ...
    const imageNode = () => {
      if (children.length === 1) {
        ...
        return <Icon_default code={60057} fontSize={imageWidth} color={imageColor} />;
      }
    };
    return <div className={classPrefix2}>
      <div className={`${classPrefix2}-image-container`}>{imageNode()}</div>
      {props.description && <span style="color:#ccc;font-size:14px;margin-top:8px">{props.description}</span>}
    </div>;
  };
  css = () => {
    return `
