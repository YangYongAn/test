...
var Input = class extends Component {
  ...
  render = (props) => {
    ...
    return <div className={`${classPrefix2}-wrapper`} {...props}>
      ...
      })} style={iptStyles} placeholder-style={`color: ${placeholderColor}`} placeholder-class={placeholderClass} placeholder={placeholder} disabled={!!(disabled || readOnly)} maxLength={maxLength} minLength={minLength} max={max} min={min} autoComplete={autoComplete} enterKeyHint={enterKeyHint} pattern={pattern} type={type} autoCapitalize={autoCapitalize} autoCorrect={autoCorrect} onKeyDown={handleKeydown} onKeyUp={onKeyUp} onInput={(e) => {
      ...
      {clearable && !!this.data.value && this.data.hasFocus && <div className={`${classPrefix2}-clear`} onClick={() => {
        this.setValue("");
        handleClear?.();
      }}><Icon_default code={59939} fontSize="16px" /></div>}
    </div>;
  };
  css = () => {
    return `
