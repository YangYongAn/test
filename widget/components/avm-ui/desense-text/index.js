...
var classPrefix2 = "adm-desense-text";
var defaultProps = {
  defaultDesense: true
};
var DesenseText = class extends Component {
  data = {
  ...
  render = (props) => {
    ...
    return <div className={classPrefix2}>
      <text>{this.data.desense ? desenseText : text}</text>
      <div onClick={() => {
        this.data.desense = !this.data.desense;
        props.onChange && props.onChange(this.data.desense);
      }} className={`${classPrefix2}-icon-wrap`}>{this.data.desense ? <Icon_default code={59963} /> : <Icon_default code={59964} />}</div>
    </div>;
  };
  css = () => {
    return `
...
export {
  desense_text_default as default
};
