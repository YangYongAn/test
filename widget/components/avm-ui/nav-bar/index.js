...
var NavBar = class extends Component {
  render = (props) => {
    ...
    return <div className={classNames(classPrefix2)} style={style}>
      <div className={`${classPrefix2}-left`} role="button">
        {back !== null && <div className={`${classPrefix2}-back`} onClick={props.onBack}>
          {backArrow && <div className={`${classPrefix2}-back-arrow`}>{backArrow === true || backArrow === "true" ? <Icon_default code={59988} fontSize="18" /> : backArrow}</div>}
          <text aria-hidden="true">{back}</text>
        </div>}
        <text className={`${classPrefix2}-left-text`}>{props.left}</text>
      </div>
      {props.children && Boolean(props.children) && typeof props.children[0] === "string" ? <text className={`${classPrefix2}-title`}>{props.children}</text> : <div className={`${classPrefix2}-title`}>{props.children}</div>}
