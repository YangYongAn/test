...
// packages/avm-ui/src/components/result/result.tsx
var classPrefix2 = `adm-result`;
...
var Result = class extends Component {
  render = (props) => {
    ...
    return <div className={classNames(classPrefix2, `${classPrefix2}-${status}`)}>
      <div className={`${classPrefix2}-icon`}>{resultIcon}</div>
      <text className={`${classPrefix2}-title`}>{title}</text>
      {description ? <text className={`${classPrefix2}-description`}>{description}</text> : null}
    </div>;
  };
  css = () => {
    return `
...
// packages/avm-ui/src/components/result/index.ts
var result_default = Result;
avm.define("avm-result", Result);
export {
