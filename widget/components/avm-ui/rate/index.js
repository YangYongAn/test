...
var Rate = class extends Component {
  data = {
  ...
  render = (props) => {
    ...
    const renderStar = (v, half) => {
      return <div className={(0, import_classnames.default)(`${classPrefix2}-star`, {
        [`${classPrefix2}-star-active`]: this.data.value >= v,
        [`${classPrefix2}-star-half`]: half,
        [`${classPrefix2}-star-readonly`]: props.readOnly
      })} style={{ ...styles, color: this.data.value >= v ? activeColor : "#ddd" }} onClick={() => {
      ...
      }}>{props.character ? formatLabel(props.character, "", { color: this.data.value >= v ? activeColor : "#ddd", fontSize: starSize }) : <Icon code={60034} {...{ color: this.data.value >= v ? activeColor : "#ddd", fontSize: starSize }} />}</div>;
    };
    return <div className={classPrefix2}>{starList.map((_, i) => <div key={i} className={(0, import_classnames.default)(`${classPrefix2}-box`)}>
...
// packages/avm-ui/src/components/rate/index.ts
