...
var classPrefix2 = `adm-image`;
var defaultProps = {
  fit: "fill",
  placeholder: <div className={`${classPrefix2}-tip`}><Icon_default code={60009} color="#ddd" /></div>,
  fallback: <div className={`${classPrefix2}-tip`}><Icon_default code={60010} color="#ddd" /></div>
};
var Image = class extends Component {
  ...
  render = (props) => {
    ...
    const renderInner = () => {
      ...
      const imgs = () => <img className={`${classPrefix2}-img`} src={src} alt={props.alt} onClick={() => {
      ...
      return <view className={`${classPrefix2}-wrapper`}>
        {imgs()}
        <div className={!this.data.loaded ? `${classPrefix2}-placeholder-wrapper` : ""}>{!this.data.loaded && props.placeholder}</div>
      </view>;
    };
    return <div className={classPrefix2} style={style}>{renderInner()}</div>;
  };
  css = () => {
    return `
      .adm-image {
