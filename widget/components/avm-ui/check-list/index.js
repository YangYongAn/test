...
var CheckListItem = class extends Component {
  render = (props) => {
    const active = props.checked ? props.checked.includes(props.value) : false;
    const readOnly = props.readOnly;
    const activeIcon = props.activeIcon;
    const extra = <div className={`${classPrefix4}-extra`}>{active ? activeIcon ?? <Icon type="CheckCircleFill" color={active ? "#9AC200" : "#666"} /> : ""}</div>;
    return <ListItem title={props.title} className={classNames({
      [`${classPrefix4}-readonly`]: readOnly
    })} active={active} description={props.description} prefix={props.prefix} handleClick={() => {
      if (readOnly)
        return;
      if (active) {
    ...
    }} arrow={false} clickable={!readOnly} extra={extra} disabled={props.disabled}>{props.label}</ListItem>;
  };
};
...
var CheckList = class extends Component {
  data = {
  ...
  render = (props) => {
    ...
    const childEle = list.map((item) => {
      ...
      return <CheckListItem {...item} disabled={disabled} readOnly={readOnly} checked={this.data.checked} activeIcon={props.activeIcon} check={() => this.check(item.value, props)} uncheck={() => this.uncheck(item.value, props)} />;
    });
    return <List_default mode={props.mode}>{childEle}</List_default>;
  };
  css = () => {
    return `
...
// packages/avm-ui/src/components/check-list/index.ts
var check_list_default = CheckList;
avm.define("avm-check-list", CheckList);
export {
  check_list_default as default
};
