...
var ActionSheet = class extends Component {
  data = {
    borderRadiusStyle: {
  ...
  render = (props) => {
    ...
    const btnListEle = props.actions.map((action, index) => {
      ...
      return <div key={key} className={`${classPrefix4}-button-item-wrapper`}><div onClick={() => {
        if (disabled !== true) {
      ...
      }} className={classNames(`${classPrefix4}-button-item`, {
        [`${classPrefix4}-button-item-danger`]: danger,
        [`${classPrefix4}-button-item-disabled`]: disabled
      })}>
        <span className={classNames(`${classPrefix4}-button-item-name`, {
          [`${classPrefix4}-button-item-danger-name`]: danger
        })}>{text}</span>
        {description && <span className={`${classPrefix4}-button-item-description`}>{description}</span>}
      </div></div>;
    });
    return <Popup_default bodyStyle={this.data.borderRadiusStyle} bodyHeight="auto" visible={props.visible} maskClick={() => {
      props.maskClick?.();
      if (props.closeOnMaskClick) {
        props.handleClose?.();
      }
    }} afterClose={props.afterClose} className={`${classPrefix4}-popup`} getContainer={props.getContainer}><div className={classPrefix4}>
      {(props.extra || props.extraDesc) && <div className={`${classPrefix4}-header`}>
        {props.extra && formatLabel(props.extra, `${classPrefix4}-extra`)}
        {props.extraDesc && formatLabel(props.extraDesc, `${classPrefix4}-extra-desc`)}
      </div>}
      <div className={`${classPrefix4}-button-list`}>{btnListEle}</div>
      {props.cancelText && <div className={`${classPrefix4}-cancel`}><div className={`${classPrefix4}-button-item-wrapper`}><div className={`${classPrefix4}-button-item`} onClick={() => {
        props.handleClose && props.handleClose();
      }}>{formatLabel(props.cancelText, `${classPrefix4}-button-item-cancel`)}</div></div></div>}
    </div></Popup_default>;
  };
  css = () => {
    return `
...
// packages/avm-ui/src/components/action-sheet/index.ts
